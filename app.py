...
if not st.session_state.logged_in:
    ...
    with tab2:
        ...
        if st.button("Signup"):
            if username_exists(new_username):
                ...
                add_user(new_username, hash_password(new_password), email)
    ...
    score_predictor = pickle.load(open('score.pkl', 'rb'))
            ...
            if target >= 0 and target <= 300 and overs >= 0 and overs <= 20 and wickets <= 10 and wickets >= 0 and score >= 0:
                try:
                    if st.button('Predict Probability'):
                        ...
                        requiredrunrate = (runs_left * 5) / balls_left if balls_left > 0 else 0
            ...
            if battingteam == bowlingteam:
                ...
                try:
                    if st.button('Predict Score'):
                        ...
                        # Loading the trained machine learning pipeline to make the prediction
                        predicted_score = score_predictor.predict(input_df)[0]
